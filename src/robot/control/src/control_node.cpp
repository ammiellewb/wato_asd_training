...
ControlNode::ControlNode(): Node("control"), control_(robot::ControlCore(this->get_logger())) {
  ...
  // Initialize control
  control_.initializeControlCore(
    lookahead_distance_,
    max_steering_angle_,
    steering_gain_,
    linear_velocity_);
...
void ControlNode::getParameters() {
...
void ControlNode::followPath() {
  if (control_.isPathEmpty()) {
    RCLCPP_WARN_THROTTLE(this->get_logger(), *this->get_clock(), 3000, "Path is empty.");
    return;
  }

  geometry_msgs::msg::Twist control_command = control_.computeControlCommand(
    robot_x_, robot_y_, robot_theta_);
  cmd_pub_->publish(control_command);
}

void ControlNode::timerCallback() {
  if (robot_x_ == 0 && robot_y_ == 0) {
    RCLCPP_WARN_THROTTLE(this->get_logger(), *this->get_clock(), 3000, "Robot position is not set.");
    return;
  }
...
double ControlNode::quaternionToYaw(
